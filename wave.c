...
static float loudness_attn_factor(const float samplepercent, const struct AttnTab* Tab, const size_t* offset) {
	float Factor = 0;
...
static unsigned int sample_end_percent_value(const size_t whole_process_len, const struct AttnTab* Tab,
					     const size_t* offset, const size_t wrote_len) {
	return (Tab->Attntab[*offset + 1]) * whole_process_len;
}

#define UPDATE_OFFSET_CHECK(sample_loop_counter, attn_fac_index, attnsrc)                                              \
	if (Loudness_Offset_Add4_sample_phase[attn_fac_index] < sample_loop_counter + wrote_len) {                     \
		attn_fac_offset_tmpvar[attn_fac_index] += 4;                                                           \
		Loudness_Offset_Add4_sample_phase[attn_fac_index] = sample_end_percent_value(                          \
		    whole_process_len, attnsrc + attn_fac_index, attn_fac_offset_tmpvar + attn_fac_index, wrote_len);  \
	}
...
#define UPDATE_OFFSET_CHECK_ENVELOP(sample_loop_counter, attn_fac_index, attnsrc)                                      \
	if (Loudness_Offset_Add4_sample_phase[attn_fac_index] < sample_loop_counter + wrote_len) {                     \
		attn_fac_offset_tmpvar[attn_fac_index] += 4;                                                           \
		Loudness_Offset_Add4_sample_phase[attn_fac_index] = sample_end_percent_value(                          \
		    whole_process_len, attnsrc, attn_fac_offset_tmpvar + attn_fac_index, wrote_len);                   \
	}
...
static void WriteWave_Worker(float* Buffer, const struct WaveTab* wt, const struct WaveArgs* arg, const size_t len,
			     size_t pending_len, size_t wrote_len) { // len refers to array element numbers.
	size_t whole_process_len = arg->total_len;		     // 处理整个音符的采样长度
	// 用前需memset();

	float percent = (float)(wrote_len) / (float)whole_process_len;
	...
	for (unsigned int i = 0, j = 0; i < wt->Tab2TLen; i += 2, j++) {
		attn_fac_offset_tmpvar[j] =
		    RangeFArrayLookup((wt->Attn + j)->Attntab, percent, (wt->Attn + j)->TabLen, 4);
	}
	attn_fac_offset_tmpvar[MAX_WAVETAB_LEN] = RangeFArrayLookup(arg->Attn.Attntab, percent, arg->Attn.TabLen, 4);
	Loudness_Offset_Add4_sample_phase[MAX_WAVETAB_LEN] = sample_end_percent_value(
	    whole_process_len, &arg->Attn, attn_fac_offset_tmpvar + MAX_WAVETAB_LEN, wrote_len);

	for (unsigned int i = 0; i < wavetab_tab2tlen_div_2; i++) {
		Loudness_Offset_Add4_sample_phase[i] =
		    sample_end_percent_value(whole_process_len, wt->Attn + i, attn_fac_offset_tmpvar + i, wrote_len);
	}
	...
	for (unsigned int i = 0; i < pending_len; i++) {
		...
		for (unsigned int j = 0, k = 0; j < wt->Tab2TLen; j += 2, k++) {
			sample = fastsin((i + wrote_len) * arg->freq * wt->Tab2T[j] + phi[k]);
...
void WriteWave(float* Buffer, struct WaveTab* wt, struct WaveArgs* arg, size_t len) {
	...
	if (wt->Tab2TLen / 2 > MAX_WAVETAB_LEN) {
		printf("Due to the need of calucating relative Loudness to keep envelop not affected by time-based "
		       "frequency domain functon.\n");
...
#pragma omp parallel proc_bind(close)
	{
		...
		WriteWave_Worker(Buffer + thread_wrote_offset[tid], wt, arg, len, thread_pendinglen[tid],
