...
#define SWAP32BIT(x)                                                                                                   \
	x = (((unsigned int)x) << 24) | ((((unsigned int)x) << 8) & 0x00ff0000) |                                      \
	    ((((unsigned int)x) >> 8) & 0x0000ff00) | (((unsigned int)x) >> 24)
...
static struct pa_simple* PulseAudio_Init() { // the only backend.
	...
	return pa_simple_new(NULL, "MusicPlayer", PA_STREAM_PLAYBACK, NULL, "Player", &spec, NULL, NULL, NULL);
}
